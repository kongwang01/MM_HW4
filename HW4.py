    ...
    if mode == "ColorHistogram":
        ...
        if (is_there_pickle == False):
            for i in range(0, images_quantities):
                ...
                d = 0
                Zmod = 1
                #direction = [0,1,2,3]  右，左下，下，右上 (Zmod = 1)
                #direction = [0,1,2,3]  右，右上，下，左下 (Zmod = 2)
                while(True):
                    #print(Zx , ",", Zy)
                    all_images_dictionary[i]["DCT_Y_list"].append(DCT_Y[Zx][Zy])
                    all_images_dictionary[i]["DCT_Cb_list"].append(DCT_Cb[Zx][Zy])
                    all_images_dictionary[i]["DCT_Cr_list"].append(DCT_Cr[Zx][Zy])
                    if Zx == 7 and Zy == 7:
                        break
                    if Zx == 0 and Zy == 7:
                        Zmod = 2
        ...
        for block_x in xrange(number_of_grids_n):    
            for block_y in xrange(number_of_grids_n):
                ...
                while(True):
                    #print(Zx , ",", Zy)
                    mosaic_images_dictionary[mosaic_curr_block_key]["DCT_Y_list"].append(DCT_Y[Zx][Zy])
                    mosaic_images_dictionary[mosaic_curr_block_key]["DCT_Cb_list"].append(DCT_Cb[Zx][Zy])
                    mosaic_images_dictionary[mosaic_curr_block_key]["DCT_Cr_list"].append(DCT_Cr[Zx][Zy])
                    if Zx == 7 and Zy == 7:
                        break
                    if Zx == 0 and Zy == 7:
                        Zmod = 2
        ...
        for x in xrange(number_of_grids_n):    
            for y in xrange(number_of_grids_n):
                ...
                for i in range(0, images_quantities):
                    ...
                    ith_Cr_distance = 0
                    for j in range(0, 64):
                        ith_Y_distance += (((mosaic_images_dictionary[curr_block_key]["DCT_Y_list"][j] - all_images_dictionary[i]["DCT_Y_list"][j])*(64-j)/Zweight)**2)
                        ith_Cb_distance += (((mosaic_images_dictionary[curr_block_key]["DCT_Cb_list"][j] - all_images_dictionary[i]["DCT_Cb_list"][j])*(64-j)/Zweight)**2)
                        ith_Cr_distance += (((mosaic_images_dictionary[curr_block_key]["DCT_Cr_list"][j] - all_images_dictionary[i]["DCT_Cr_list"][j])*(64-j)/Zweight)**2)
        ...
        #確認是否已經有pickle檔
        pickle_path = 'HW4_AverageRGB_' + str(number_of_grids_n) + 'x' + str(number_of_grids_n) + '.pickle'
        if os.path.exists(pickle_path):
                is_there_pickle = True
                with open(pickle_path, 'rb') as handle:
                    all_images_dictionary = pickle.load(handle)
        
        #讀入所有jpg的AverageRGB(只有第一次按Search時需要)
        if (is_there_pickle == False):
            for i in range(0, images_quantities):
                temp_str = str(i)
                temp_str_fillZero = temp_str.zfill(4)
                ...
                averageR /= width*height
                averageG /= width*height
                averageB /= width*height

                all_images_dictionary[i]["Average_R"] = averageR
                all_images_dictionary[i]["Average_G"] = averageG
                all_images_dictionary[i]["Average_B"] = averageB
                

            #輸出pickle
            output_pickle_path = 'HW4_AverageRGB_' + str(number_of_grids_n) + 'x' + str(number_of_grids_n) + '.pickle'
            with open(output_pickle_path, 'wb') as handle:
                pickle.dump(all_images_dictionary, handle, protocol=pickle.HIGHEST_PROTOCOL)
        #-------------------------------------------
        ...
        for x in xrange(width):    
            for y in xrange(height):
                ...
                if curr_block_key not in mosaic_images_dictionary:
                    mosaic_images_dictionary[curr_block_key] = {}
                    mosaic_images_dictionary[curr_block_key]["Average_R"] = 0
                    mosaic_images_dictionary[curr_block_key]["Average_G"] = 0
                    mosaic_images_dictionary[curr_block_key]["Average_B"] = 0
                    mosaic_images_dictionary[curr_block_key]["Average_R_count"] = 0
                    mosaic_images_dictionary[curr_block_key]["Average_G_count"] = 0
                    mosaic_images_dictionary[curr_block_key]["Average_B_count"] = 0
                    
                R, G, B = pixel[x,y]
                mosaic_images_dictionary[curr_block_key]["Average_R"] += R
                mosaic_images_dictionary[curr_block_key]["Average_G"] += G
                mosaic_images_dictionary[curr_block_key]["Average_B"] += B
                mosaic_images_dictionary[curr_block_key]["Average_R_count"] += 1
                mosaic_images_dictionary[curr_block_key]["Average_G_count"] += 1
                mosaic_images_dictionary[curr_block_key]["Average_B_count"] += 1
                    
        
        for k in mosaic_images_dictionary.keys():
            mosaic_images_dictionary[k]["Average_R"] /= mosaic_images_dictionary[k]["Average_R_count"]
            mosaic_images_dictionary[k]["Average_G"] /= mosaic_images_dictionary[k]["Average_G_count"]
            mosaic_images_dictionary[k]["Average_B"] /= mosaic_images_dictionary[k]["Average_B_count"]
        #-------------------------------------------
        print "calculate_distance"
        distance_QueryWithImages = list()
        for x in xrange(number_of_grids_n):    
            for y in xrange(number_of_grids_n):
                curr_block_key = "block" + str(x).zfill(4)+ str(y).zfill(4)
                for i in range(0, images_quantities):
                    distance_QueryWithImages.append(list())  #distance_QueryWithImages[i]
                    ith_distance = 0
                    ith_R_distance = 0
                    ith_G_distance = 0
                    ith_B_distance = 0
                    ...
                    #print ith_distance
                    ith_R_distance = ((mosaic_images_dictionary[curr_block_key]["Average_R"] - all_images_dictionary[i]["Average_R"])**2) ** (0.5)
                    ith_G_distance = ((mosaic_images_dictionary[curr_block_key]["Average_G"] - all_images_dictionary[i]["Average_G"])**2) ** (0.5)
                    ith_B_distance = ((mosaic_images_dictionary[curr_block_key]["Average_B"] - all_images_dictionary[i]["Average_B"])**2) ** (0.5)
                    ith_distance = ith_R_distance + ith_G_distance + ith_B_distance
                    distance_QueryWithImages[i].append(i)
                    distance_QueryWithImages[i].append(ith_distance)
                    #print curr_block_key,i,ith_distance
